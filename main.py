...
print("-----------------------------------------------------------------")
# 정상과 당뇨 환자가 각각 몇 명인지 확인    (정상 500명, 당뇨병 환자 268명)
...
# 그래프 속성
'''
    시본 라이브러리 중 각 항목 간 상관관계를 나타내는 heatmap() 함수 통해 그래프 표사
...
# plasma 를 기준으로 각각 정상과 당뇨가 어느 정도 비율 분포 확인하기
'''
    hist() 함수 안에 x축 지정 (가져올 칼럼)
        - diabetes 값이 0인 것과 1인 것을 구분해 불러옴
        - bins : x 축을 몇개의 막대로 쪼개어 줄 것인지 정함
...
# bmi 를 기준으로 각각 정상과 당뇨가 어느 정도 비율로 분포하는지 확인하기
'''
    BMI 수치가 높아질 경우 당뇨의 발병률도 함께 증가하는 추세임
    평균이나 중앙값으로 대치하거나, 이상치를 제거 과정 등이 데이터 전처리에 포함 가능(성능 높임)
